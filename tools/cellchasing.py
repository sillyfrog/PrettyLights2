...
# "First" chaser
run(
    f"./chaserlights.py 1500 --off {OFF} --on {ON} --fadeon 0 --ontime {ONT} --fadeoff={FADE}  --waittime {SPACING} --color '#000000ff' --offset 0 cell1.png"
)
# Second chaser, dimmer
run(
    f"./chaserlights.py 1500 --off {OFF} --on {ON2} --fadeon {FADE} --ontime {ONT} --fadeoff=0  --waittime {SPACING} --color '#000000ff' --offset 0 cell2.png"
)

# Merge together for the first strip, on transparent background so can be merged with reverse
# Make something to write to
for i in range(4):
    run(
        f"./chaserlights.py 1500 --off '#00000000' --on '#00000000' --fadeon 0 --ontime 0 --color '#00000000' cellchaser{i}.png"
    )
    run(
        f"./repeatpattern.py 1500 cell1.png cellchaser{i}.png --offset {i * ONT} --waittime {SPACING} --pattern 0 --color '#00000000'"
    )
    run(
        f"./repeatpattern.py 1500 cell2.png cellchaser{i}.png --offset {ONT * (7-i)} --waittime {SPACING} --pattern 0 --color '#00000000'   --stripalpha"
    )
